...
for (const mode of ["repaired", "fabricated", "unavailable", "all-withheld", "valid-only", "duplicate-id", "new-id", "wrong-line"] as const) {
  test(`citation correction preserves supported peers: ${mode}`, async () => {
    ...
    const good = { citationId: "repo:read:1", line: 1, detail: "The current implementation uses a literal decision." };
    const bad = { ...good, citationId: "invented:999" };
    ...
    const review = async <T>(request: ModelReviewRequest) => {
      if ((request.schema.properties as Record<string, unknown>)?.repairs) {
        ...
        return { output: { repairs: [{ id: "broken", evidence: [mode === "repaired" ? { ...good, line: 2 } : bad] }] } as T, provider: "fixture", model: "fixture" };
      }
      return { output: {
        schemaVersion: 1,
        overall: { verdict: "ready-with-minor-improvements", risk: "low", ship: true, summary: "The implementation has a few localized policy ownership improvements.", primaryConcern: "" },
    ...
    const model: ReviewModel = repositoryReviewModel(["index.ts"], review);
