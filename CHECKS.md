...
| **Examples** | Accepting `ctx` but coercing nil to `Background`/`TODO`; fixing one HTTP path and leaving its twin half-done; adding parameters without threading call sites; nil-guard on init/config that only fires under an extra condition (e.g. another spec nil check) |
