...
function isReviewableSource(path) {
  ...
  if (segments.includes("integration") && /^(?:00-setup|99-teardown)$/.test(name)) {
    return true;
  }
...
- Host-destructive test harnesses: report one harness-level operational-risk concern only when prepared sources jointly prove all of the following: a changed test or integration harness is directly runnable on a developer host through a documented or default local entrypoint; that entrypoint reaches privileged persistent host mutation such as replacing host configuration, installing host binaries or packages, or changing persistent service state; the same reachable harness controls live services or global workloads; and no dominating boundary before the first effect requires CI, explicit destructive-test opt-in, a disposable container or VM, or refusal when an existing installation or service is detected. Cite the local entrypoint, the persistent host effect, and the service or global-workload effect. Recommend a proportionate CI-only refusal, explicit opt-in that names the effects, disposable environment, or existing-service refusal. Stay quiet for Dockerfile/image-build effects; scripts reachable only from an ephemeral CI job; effects proven to run inside a disposable container, VM, chroot, rootfs, or filesystem namespace; test-owned temporary storage or mocks; inert documentation, fixtures, or uninvoked helpers; a dominating CI/opt-in/existing-service refusal before every effect; or source that proves only mutation without service/global control (or only service control without persistent mutation). Never infer host reachability from filenames, words such as setup or integration, sudo, system paths, or service commands alone.
